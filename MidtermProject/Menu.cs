...
namespace MidtermProject
{
    class Menu
    {
        ...
        public static void SelectPayment(string howDoYouWantToPay, double amount) //<-- change this from void to payment once it's created 
        ...
        string paymentType = Console.ReadLine(); //<-- I think we should move this method to the program
            ...
            if (paymentType == "creditCard")
            {
                ...
                credit1.PayWithCredit(amount); //<== this method needs to take in the amount
            }
            else if (paymentType == "check")
            {
                int checkNumber = int.Parse(GetUserInput("Please input your check number"));
                Check check1 = new Check(amount, checkNumber);
                check1.PayWithCheck(amount, checkNumber); //<== this method need to take in amount and check#
